...
@Controller
@Scope("session")
@ACFgFunction(id="APFF105")
@RequestMapping(value=APFtMapping.APFF105)
public class APFc105 extends ACFaAppController {
   ...
   // @Autowired APFsFuncGp funcGpService; //click the object and click import
    ...
    private class Search extends ACFgSearch {
        public Search() {
            super();
            setCustomSQL("select * from "
            		+ "(select  t1.payment_form_no, t1.request_date, t1.supplier_code, t3.supplier_name, t2.invoice_no, t1.section_id, "
            		+ "t2.sequence_no, t2.invoice_no, t1.despatch_date, t2.programme_no, t5.programme_name, "
            		+ "t2.from_episode_no, t2.to_episode_no, t2.particulars, "
            		+ "t4.section_name,  t2.payment_amount "
                    + "from arc_payment_request t1, "
                    + "arc_payment_details t2,"
                    + "arc_supplier t3,"
                    + "arc_section t4,"
                    + "arc_programme_master t5 "  
                    ...
                    + "(t4.sub_section_id = t2.sub_section_id)) "); 
     ...
     //         setModel(ARCmSupplier.class);
        	...
        	public Search setValues(ACFgRequestParameters param) throws Exception {
        		...
        		if(!param.isEmptyOrNull("select_to_date")) {
        			...
        			wheres.and("request_date", ACFdSQLRule.RuleCondition.LT, new Timestamp(param.get("select_to_date", Long.class) + 24*60*60*1000));
        		}
    ...
    @RequestMapping(value=APFtMapping.APFF105_MAIN, method=RequestMethod.GET)
    public String main(ModelMap model) throws Exception {
  ...
  //      model.addAttribute("SectionNo", sectionService.getAllSectionName34());
    ...
    @RequestMapping(value=APFtMapping.APFF105_SEARCH_AJAX, method=RequestMethod.POST)
    @ResponseBody
    ...
    @RequestMapping(value=APFtMapping.APFF105_GET_SECTION_NAME_AJAX, method=RequestMethod.POST)
    @ResponseBody
    public ACFgResponseParameters getSectionName(@RequestBody ACFgRequestParameters param) throws Exception {
     ...
     @RequestMapping(value=APFtMapping.APFF105_GET_FORM_AJAX, method=RequestMethod.POST)
        @ResponseBody
